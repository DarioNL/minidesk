<div id="editModal" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="POST" @auth('admins') action="/admin/estimates/{{$estimate->id}}/edit" @else action="/company/estimates/{{$estimate->id}}/edit" @endif>
                                ...
                                @if(count($companies))
                                    <select name="company" class="form-control">
                                        @foreach($companies as $company)
                                            <option @if($company->id == $estimate->$company) selected @endif value="{{$company->id}}" title="">{{$company->name}}</option>
                                        @endforeach
                                    </select>
                                @else
